...
async function timedFetch(url, opts, ms = 120000) {
  const ac = new AbortController();
  const t  = setTimeout(() => ac.abort(), ms);
  try {
    const r = await (await import('node-fetch')).default(url, { ...opts, signal: ac.signal });
...
export class BaseAgent {
  ...
  async _gemini(msg, history) {
    ...
    while (calls < this.maxToolRounds) {
      const r = await fetch(`${G_BASEURL}/${G_MODEL}:generateContent?key=${process.env.GEMINI_API_KEY}`, {
        method:'POST', headers:{ 'Content-Type':'application/json' },
        body: JSON.stringify({
          system_instruction:{ parts:[{ text:this.systemPrompt }] }, contents, ...tc,
          generationConfig:{ temperature:this.temperature, maxOutputTokens:2000 }
        })
      });
      ...
      if (fp) {
        ...
        if (this.requiresApproval.includes(name)) {
          return { status:'awaiting_approval', pendingApproval:{ toolName:name, toolArgs:args }, toolCallCount:calls, executionLog:log, contents, message:`Need approval for: **${name}**` };
        }
      ...
      if (tp) {
        ...
        return { status:'complete', answer:tp.text, toolCallCount:calls, executionLog:log, contents };
      }
  ...
  async _ollama(msg, history) {
    ...
    while (calls < this.maxToolRounds) {
      ...
      const r = await timedFetch(`${baseUrl}/api/chat`, {
        method:'POST', headers:{ 'Content-Type':'application/json' },
        body: JSON.stringify({ model, messages, stream:false, options:{ temperature:this.temperature, num_predict:1000 } })
      }, 120000);
      ...
      if (tc) {
        ...
        this.onToolCall?.({ name, args });
        if (this.requiresApproval.includes(name)) {
          return { status:'awaiting_approval', pendingApproval:{ toolName:name, toolArgs:args||{} }, toolCallCount:calls, executionLog:log, contents:messages, message:`Need approval for: **${name}**` };
        }
        const result = await executeTool(name, args || {});
        log.push({ step:calls, tool:name, success:result.success, ts: new Date().toISOString() });
        messages.push({ role:'assistant', content });
        messages.push({ role:'user', content:`Tool "${name}" result:\n${JSON.stringify(result,null,2)}\n\nContinue or give final answer.` });
        continue;
      }
  ...
  async resumeAfterApproval(state, approved, feedback = '') {
    ...
    const result = approved
      ...
      : { success:false, error:`Denied. ${feedback}` };

    const continueMsg = approved
      ? 'Action executed. Continue and summarize.'
      : `Denied: "${feedback}". Suggest alternatives.`;
    ...
    if (p === 'gemini') {
      ...
      const updated = [
        ...contents,
        { role:'assistant', content: JSON.stringify({ tool:toolName, args:toolArgs }) },
