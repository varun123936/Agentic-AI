...
import { executeToolsParallel }               from '../mcp/tool-executor.js';
import { getToolNames }                       from '../mcp/tool-registry.js';
import { AI_CONFIG }                          from '../config/ai.config.js';

const router = express.Router();
const SYS    = 'You are a helpful AI assistant. Use tools to get real data. Never guess.';
...
router.post('/chat', async (req,res) => {
  ...
  try {
    const t = Date.now();
    const r = await callAIWithTools(message, SYS, history);
...
router.post('/parallel', async (req,res) => {
  ...
  if (!Array.isArray(tools) || !tools.length) return res.status(400).json({ error:'tools[] required' });
  const t = Date.now();
  const r = await executeToolsParallel(tools);
  res.json({ success:true, data:{ results:r, ms:Date.now()-t } });
});
